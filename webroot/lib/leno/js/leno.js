...
(function(leno) {
	...
	leno.profile = function(opts) {
		...
		var getPos = function(opts, showTop, showLeft) {
			...
			if(orientation == 'vertical') {
				...
				var movex = -20+leno.width(toggle)/2;
				var movey = 20;
				if(showTop) {
			...
			// 箭头在左右的情况
				if(tp.x < leno.scrollLeft() + opts.width - 40) {
					showLeft = true;
				}
				var movy = 0;
				var movx = 0;
				if(showLeft) {
					...
					var position = {
						y: tp.y + leno.height(toggle)/2,
						x: tp.x - opts.width - 20
					};
				}
				var halfTop = tp.y - opts.height/2 - leno.scrollTop();
				if(opts.height < leno.clientHeight() && halfTop > 0) {
				...
				opts.node.find('.arrow').css(
					'margin-top', -(opts.height - movl + 20)
				);
			}
			return position;
		}
				...
				opts.callback.beforeShow = function(l, after) {
					if(typeof beforeShow == 'function') {
						beforeShow(l, function(layer) {
							layer.setPosition(getPos(l.opts, showTop));
							layer.opts.toggle.css('z-index', '10000');
							after(layer);
						});
					} else {
						l.setPosition(getPos(l.opts, showTop));
						l.opts.toggle.css('z-index', '10000');
						after(layer);
					}
				}
	...
	leno.hiddenBox = function(opts) {
		var box = function(opts) {
			...
			if(opts.showType == null) {
				opts.showType = 'mouseenter';
			}
			opts.trigger.css('position', 'relative');
			if(opts.showType == 'mouseenter') {
	...
	leno.scrollTo = function(node, timeout) {
		if(timeout == null) {
			timeout = 30;
		}
	...
	var layer = function(opts) {
		this.init = function(opts) {
			...
			if(this.opts.close) {
				...
				.click(function() {
					_this.hide();
				}).append('<span class="zmdi zmdi-close-circle"></span>')
...
leno.editor = (function() {
	...
	lenoEditor.prototype.resize = function(width, height) {
		...
		root.style.width = w;
		root.style.height = h;
		toolbar.style.width = root.offsetWidth - 2;
		toolbar.style.left = leno.position(root).x;
